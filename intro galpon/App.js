  ...
  return (
    <group>
      <mesh position={[0, 1.4, -8]}>
        <planeGeometry args={[4.9, 2.7]} {...props} />
...
export default function App() {
  ...
  const [{ myColor }, setLeva] = useControls(() => ({
    myColor: ColorPickerPlugin({ color: '#00b500' })
  }))

  return (
    <>
      <Canvas concurrent gl={{ alpha: false }} pixelRatio={[1, 1.5]} camera={{ position: [0, 0, 0], fov: 19}}>
        <color attach="background" args={[myColor.color]} />
        <fog attach="fog" args={['black', 19, 48]} />
        <Suspense fallback={null}>
          <group position={[0, -1, 0]}>
            
            <Memorial1 rotation={[0, 1.5, 0]} position={[6, 3.7, -4]} scale={[1.66, 1.3, 1.26]} />
            {/*<Camera rotation={[0.4, 0.7, 0]} position={[-3, -1, -19]} scale={[1.46, 1.96, 1.66]} />  */}
...
function Intro({ start, set }) {
  ...
  return useFrame((state) => {
    if (start) {
      state.camera.position.lerp(vec.set(state.mouse.x * 5, 3 + state.mouse.y * 2, 14), 0.04)
